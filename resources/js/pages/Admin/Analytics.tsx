...
import type { DashboardStats, PageProps, PaginatedData, SerpFetch } from '../../types';
...
export default function Analytics() {
    const { stats, startDate, endDate, recentSerpFetches } = usePage<AnalyticsPageProps>().props;
    const [start, setStart] = useState(startDate);
    const [end, setEnd] = useState(endDate);

    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        router.get('/admin/analytics', { start, end }, { preserveState: true, preserveScroll: true });
    }
            ...
            <div className="space-y-6">
                ...
                <Card>
                    <h2 className="mb-4 text-lg font-semibold text-navy-950">
                        Visits from {formatDate(startDate)} to {formatDate(endDate)}
                    </h2>
                ...
                <Card>
                    <h2 className="mb-4 text-lg font-semibold text-navy-950">SERP fetches over the date range</h2>
                    <div className="h-72 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart
                    ...
                    <div className="mt-6">
                                    ...
                                    {recentSerpFetches.data.length === 0 && (
                                        <tr>
                                            <td colSpan={3} className="px-4 py-8 text-center text-slate-500">
                                                No SERP fetches in the selected date range.
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
                        ...
                        {recentSerpFetches.meta.last_page > 1 && (
                            <Pagination start={start} end={end} meta={recentSerpFetches.meta} />
                        )}
                    </div>
                </Card>
            </div>
...
function Pagination({
    ...
    meta: PaginatedData<SerpFetch>['meta'];
}) {
    return (
                    ...
                    .map((link) => (
                        <button
                            ...
                            onClick={() =>
                                router.get(
                                    '/admin/analytics',
                                    { start, end, page: Number(link.label) },
